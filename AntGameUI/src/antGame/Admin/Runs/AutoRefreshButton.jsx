...
const AutoRefreshButton = props => {
  ...
  return (
    <span
      className={`${adminStyles.divButton} ${styles.autoRefresh} ${enabled ? styles.enabled : null}`}
      onClick={() => {
        resetRefreshTime();
        setEnabled(!enabled);
      }}
    >
      {enabled ? (
        <Countdown ref={countdownRef} date={nextRefreshTime} onComplete={() => handleTimeout()} renderer={renderer} />
      ) : (
        "Live"
      )}
    </span>
